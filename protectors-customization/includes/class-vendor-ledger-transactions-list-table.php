...
class Vendor_Ledger_Transactions_List_Table_Updated extends Transaction_List_Table {
    public $type_id = [];
    public $chart_group  = [];
    public $account_prev_balance = 0;
    public $TotalDebit = 0;
    public $TotalCredit = 0;
    public $StockImported = array();
	
	
    function __construct() {

        $this->type = 'journal';
        $this->slug = ! empty( $_GET['page'] ) && ( $_GET['page'] == 'erp-vendor-ledger' ) ? 'erp-vendor-ledger' : 'erp-vendor-ledger';

        parent::__construct();

        \WP_List_Table::__construct([
            'singular' => 'vendor-ledger',
            'plural'   => 'vendor-ledgers',
            'ajax'     => false
        ]);
    ...
    function get_columns() {
        ...
        $columns = array(
            ...
            'slip_id' => __( 'GRN', 'erp' ),
            'ref'        => __( 'Ref', 'erp' ),
            'product_id'    => __( 'Item Code', 'erp' )
        );

            $columns['debit']   = __( 'Debit', 'erp' );
            $columns['credit']  = __( 'Credit', 'erp' );
            $columns['balance'] = __( 'Balance in', 'erp' );
			$columns['remarks'] = __( 'Remarks', 'erp' ); 
    ...
    function column_issue_date( $item ) {
        ...
        return sprintf( '<a target="_blank" href="%1$s">%2$s</a> %3$s', admin_url( 'admin.php?page=erp-accounting-expense&action=view&id=' . $item->id ), erp_format_date( $item->issue_date ), $this->row_actions( $actions ) );
    }
   ...
   /**
	...
	function column_form_type( $item ) {
		...
		$invoice_number = erp_ac_get_invoice_number( $item->id, erp_ac_get_invoice_format( $item->form_type ));
   ...
   /**
	...
	function column_slip_id( $item ) {
		...
		$StockImported = impaktt_get_imported_vouchers($item->id);
		$this->StockImported = $StockImported;
		if(is_null($StockImported))
			return '&#8212';
		else
			return $StockImported->id;
	}		


   /**
     * Remarks during import product
     *
     * @param  array $item
     *
     * @return string
	...
	function column_remarks( $item ) {
		...
		$StockImported = $this->StockImported;
		if(is_null($StockImported))
			return '&#8212';
		else
			return $StockImported->extra_comments;
	}
        ...
        if ( 'top' == $which ) {
            echo '<div class="alignleft actions">';
				...
				if(isset( $transaction['user_id'] )){
					...
					$substr = [ '' => __( '&mdash; Select &mdash;', 'erp' ) ] + erp_get_peoples_array( ['type' => 'vendor', 'number' => '50' ] );
				}							
	...
	if ( $customerargs["remaining_balance"] ) {
		erp_html_form_input([
			...
			erp_html_form_input( array(
							...
							'options'     => [ '' => __( '&mdash; Select &mdash;', 'erp' ) ] + erp_get_peoples_array( ['type' => 'vendor', 'number' => '-1' ] ),
							'custom_attr' => [
			...
			submit_button( __( 'Reset', 'erp' ), 'button', 'reset_filtered', false,array( "onclick" => "reset_function_get();"));			
    ...
    function prepare_items() {
        ...
        if ( isset( $_GET['section'] ) ) {
            ...
            $this->items = erp_ac_get_ledger_transactions( $args, 'expenses' );
            ...
            $closing    = erp_ac_get_opening_ledger( 'expenses', $start_date, $args['people_user_id']);
            $pagination = erp_ac_get_ledger_opening_pagination( $offset, 'expenses', $args );
            ...
            $total_balance->slip_id    = '&nbsp;';			
            $total_balance->ref        = sprintf( '<strong>%s</strong>',__( 'Total Credit', 'erp' ) );	
            $total_balance->product_id = '&#8212';
            $total_balance->debit      = '&nbsp;';
